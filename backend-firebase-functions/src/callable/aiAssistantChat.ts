...
export const aiAssistantChat = onCall({ secrets: [openaiApiKey] }, async (req) => {
  ...
  for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
    ...
    const rawToolCalls: OpenAI.Chat.ChatCompletionMessageToolCall[] = assistantMessage.tool_calls || [];
    ...
    if (toolCalls.every((tc: OpenAI.Chat.ChatCompletionMessageFunctionToolCall) => PROPOSAL_TOOL_NAMES.has(tc.function.name))) {
      break;
    }
  }
